...
const filteredProducts = computed(() => {
    ...
    if (activeFilters.value.stock.length > 0) {
        filtered = filtered.filter((product) => {
            if (
                activeFilters.value.stock.includes("inStock") &&
                product.inStock
            )
                return true;
            if (
                activeFilters.value.stock.includes("outOfStock") &&
                !product.inStock
            )
                return true;
            return false;
    ...
    if (
        ...
        filtered = filtered.filter(
            (product) =>
                product.price >= parseFloat(activeFilters.value.priceRange.min),
        );
    }
    if (
        ...
        filtered = filtered.filter(
            (product) =>
                product.price <= parseFloat(activeFilters.value.priceRange.max),
        );
    }
